...
class EmailSenderApp(Tk):
    def __init__(self):
        ...
        # Declare variable with name count to store recurring email count
    ...
    def send_email(self):
        ...
        if is_recurring:
            ...
            threading.Thread(target=self.schedule_recurring_email,
                             args=(send_after_seconds,)).start()
        else:
    ...
    def send_single_email(self):
        ...
        try:
            ...
            # After you have sent the recipients three consecutive emails, change the subject line to "Reminder Email!!".
    ...
    def schedule_recurring_email(self, send_after_seconds):
        # Replace count with self.count and comment count = 0
        count = 0
        # After sending 5 recurring email break the loop
        while True:
            if (count < 3):
                count += 1
